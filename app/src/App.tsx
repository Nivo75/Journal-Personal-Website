    ...
    const load = async () => {
      try {
        ...
        if (peaksRes.ok) {
          ...
          setPeaks(Array.isArray(peaksJson) ? peaksJson : (peaksJson?.items ?? []))
        }
        if (tripsRes.ok) {
          const tripsJson = await tripsRes.json()
          setTripReports(Array.isArray(tripsJson) ? tripsJson : (tripsJson?.items ?? []))
        }
        if (galleryRes.ok) {
          const galleryJson = await galleryRes.json()
          setGalleryImages(Array.isArray(galleryJson) ? galleryJson : (galleryJson?.items ?? []))
        }
